...
人物資料選擇畫面 可以匯入 但匯出沒反應(沒關係)

人物進入遊戲後 點選外掛的備份或存檔功能可以匯出並找到存檔
...
​伺服器選擇（伺服器一、伺服器二、100% 無外掛純淨版）。
​自動注入雙外掛（主外掛 + GM 免費商店，並帶有 0.5 秒延遲防止衝突）。
​玩家自訂本地外掛（檔案上傳功能）：支援沒有網址的本地 .js 檔案，選擇一次後自動透過 localStorage 記憶！
​現代化 UI 介面：質感黑金風格設計，響應式排版，完美支援手機與電腦畫面。

​支援無網址外掛：玩家只要將手中的外掛檔案（例如 my_script.js）上傳一次，啟動器會自動用存儲區記憶下來，以後每次點「🚀 啟動遊戲」都會自動將這個自訂外掛一起注入進遊戲中！
​無縫支援純淨版：如果玩家想開無外掛原版，只要下拉選擇「原版純淨模式」即可，不會觸發任何外掛腳本。
