...
public class MainActivity extends AppCompatActivity {
    ...
    private String buildSmartFileName(String rawName, byte[] bytes) {
        ...
        if (base.isEmpty() || base.contains("fable5_save")) {
            base = extractCharInfo(bytes);
        }
        if (base.isEmpty()) {
            base = "存檔_" + System.currentTimeMillis();
        }
    ...
    private String extractCharInfo(byte[] bytes) {
        try {
            ...
            String probe = text;
            Matcher pm = Pattern.compile("\"p\"\\s*:\\s*\\{").matcher(text);
            if (pm.find()) {
                int from = pm.start();
                scope = text.substring(from, Math.min(from + 3000, text.length()));
                probe = scope;
            }
            ...
            return firstMatch(probe, new String[]{"charName", "characterName", "playerName", "nickName", "nickname", "name"});
        } catch (Exception e) {
            return "";
        }
    }
    ...
    private String mapClass(String raw) {
        ...
        switch (k) {
            case "prince": case "royal": case "king": return "王子";
            case "knight": return "騎士";
            case "mage": case "wizard": return "法師";
            case "elf": return "妖精";
            case "darkelf": case "de": return "黑暗妖精";
            case "illusionist": case "il": return "幻術士";
            case "dragonknight": case "dk": return "龍騎士";
            case "warrior": case "wa": return "戰士";
            default: return "";
        }
    }
            ...
            runOnUiThread(() -> {
                try {
                    ...
                    if (!writeToDownloads(bytes, finalName)) {
                        ...
                        Toast.makeText(MainActivity.this, "✅ 角色存檔已成功匯出至 Download：" + finalName, Toast.LENGTH_LONG).show();
                    }
                } catch (Exception e) {
